...
| `ts-literal-receivers` | TS/JS built-in calls on literal receivers, such as `"x".replace()`, stop binding unrelated user functions (#163, the first step on #59; PR #244 by @csy20 is open for it) | medium | [`ts-literal-receivers.md`](ts-literal-receivers.md) |
...
| `quality-delta-unchanged-tree-zero` | `--quality-delta` reports zero on a tree identical to HEAD, whatever the temp-dir location, checkout shape or crawl flags (#228; builds on #237) | medium | [`quality-delta-unchanged-tree-zero.md`](quality-delta-unchanged-tree-zero.md) |
...
| `deps-ts-aliases-workspaces` | `--deps`, `--arch` and `--report` say when TS/JS imports went unresolved, then draw edges for tsconfig `paths` aliases and workspace packages (#220) | large | [`deps-ts-aliases-workspaces.md`](deps-ts-aliases-workspaces.md) |
| `install-discovery-mise-aqua` | Skills embedded in the binary and a `ripwire skills install` subcommand, so `wrap` and `--doctor` work for mise, aqua and every other install channel (#225) | large | [`install-discovery-mise-aqua.md`](install-discovery-mise-aqua.md) |
| `member-macro-reparse-beyond-allcaps` | The C/C++ member-macro re-parse carried past ALL-CAPS, with `--match`, `--lint` and `--slice` reading the repaired tree (builds on #135) | large | [`member-macro-reparse-beyond-allcaps.md`](member-macro-reparse-beyond-allcaps.md) |
