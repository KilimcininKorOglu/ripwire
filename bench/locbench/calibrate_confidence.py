...
import served_syms_signal                                   # docs/research/confidence-and-abstention.md §5.4:
                                                              # the pre-registered served_syms scoring procedure
...
def main():
    ...
    # docs/research/confidence-and-abstention.md §5.4 — the pre-registered served_syms procedure.
    # Reads `summary` (just built above) only to check the §5.4.1 population fingerprint; scores
    # `rows` itself. Runs on every invocation of this script, not behind a flag, so the fingerprint
