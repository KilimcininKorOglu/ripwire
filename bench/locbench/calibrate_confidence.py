...
#
# R-MARGIN ADDENDUM (lane/for-margin-resolution): this round's own §3.3 found margin_pct= = 0 for
# 74 of 74 "low" rows — not because the underlying gap is zero, but because deriveForConfidence
# (src/lexical.h) discards the measured drop whenever the ranking hits its ceiling. margin_bp= is the
# same drop at full precision (hundredths of a percent), never zeroed that way. This script now also
# reads margin_bp= (root_attrs), scores it the same way margin_pct_alone already is (secondary_signals,
# EXPLORATORY — decides nothing on its own), and separately checks the ROUND'S OWN pre-registered
# operating-point band (margin_bp_operating_point / MARGIN_BP_FALSE_WARN_MAX / _RECALL_MIN, restated
# from docs/research/confidence-and-abstention.md §5.2). Nothing about the original measurement above
# changed: margin_pct='s own contract, discrimination() and the DOP it scores are untouched.
#
...
def root_attrs( xml ):
    ...
    return dict( confidence=root.attrib.get( "confidence" ), margin_pct=as_int( "margin_pct" ),
                 # R-MARGIN (lane/for-margin-resolution): margin_bp= is the SAME drop deriveForConfidence
                 # computes, at full precision (hundredths of a percent) and never zeroed by hitCeiling —
                 # the instrument-resolution fix for the 74/74 margin_pct=0 collapse this round measures.
...
def secondary_signals( rows, metric ):
    ...
    for name, extract, direction in ( ( "coverage", lambda r: r["coverage"], -1 ),
                                      ...
                                      # R-MARGIN: the full-precision signal, scored the SAME way as
                                      # margin_pct_alone (lower margin = more miss evidence) so the two
                                      # AUROCs are directly comparable — the whole question this round asks.
                                      ( "margin_bp_alone", lambda r: r["margin_bp"], -1 ) ):
...
# R-MARGIN (lane/for-margin-resolution) pre-registered band, restated verbatim from
# docs/research/confidence-and-abstention.md §5.2: "An operating point must exist: some threshold with
# false-warn <= 0.20 at miss-recall >= 0.50." 0.20 rather than the ARB rounds' 0.10 because a WARNING can
# tolerate a higher false rate than a REFUSAL can (the doc's own reasoning, stated there in advance).
# Independent of score_abstention_calibration.py's DOP_FALSE_ABSTAIN_MAX/DOP_RECALL_MIN, which are the
# ARB abstention-calibration round's OWN stricter band (0.10/0.20) for a different question (does the
# signal know when there is no answer, not does it know when ITS OWN answer is wrong) — reusing those
# constants here would silently swap in the wrong registration.
MARGIN_BP_FALSE_WARN_MAX, MARGIN_BP_MISS_RECALL_MIN = 0.20, 0.50


def margin_bp_operating_point( rows, metric ):
    """Does ANY margin_bp threshold clear the pre-registered band above? Swept the same way
    discrimination()'s best_f1 is (every distinct score value as a candidate cut), but graded against
    THIS round's band rather than best-F1 — a best-F1 cut can sit outside the band entirely, and the
    pre-registration asks whether the band is reachable at all, not whether it is the optimum."""
    usable = [ r for r in rows if r["margin_bp"] is not None ]
    labels = [ not r[metric] for r in usable ]                 # True == miss == should have warned
    scores = [ -r["margin_bp"] for r in usable ]                # higher == more miss evidence (low margin)
    n_pos = sum( 1 for y in labels if y )
    out = dict( n=len( usable ), misses=n_pos, auroc=None, meets_band=False, best_in_band=None )
    if not ( 0 < n_pos < len( usable ) ):
        return out
    out["auroc"] = auroc( labels, scores )
    _best, table = sweep_thresholds( labels, scores )
    in_band = [ r for r in table if r["false_abstain_rate"] is not None and r["false_abstain_rate"] <= MARGIN_BP_FALSE_WARN_MAX
               and r["recall"] is not None and r["recall"] >= MARGIN_BP_MISS_RECALL_MIN ]
    out["meets_band"] = bool( in_band )
    if in_band:
        # the highest-recall candidate inside the band — the most useful operating point among the
        # ones that qualify, not the best-F1 one (which may not qualify at all).
        out["best_in_band"] = max( in_band, key=lambda r: r["recall"] )
    return out
...
def md_signal_sections( summary ):
    ...
    L += [ "", "### EXPLORATORY — two other root facts as miss detectors (decides nothing)", "",
           ...
           "| --- | --- | --- | --- |" ]
    for name in ( "coverage", "dropped_positive", "margin_pct_alone", "margin_bp_alone" ):
    ...
    # R-MARGIN: the pre-registered operating-point verdict, separate from the exploratory table above —
    # this is the round's actual question, not one more row in a "decides nothing" list.
    L += [ "", "### R-MARGIN — margin_bp= operating point vs the pre-registered band "
           "(false-warn <= %.2f at miss-recall >= %.2f)" % ( MARGIN_BP_FALSE_WARN_MAX, MARGIN_BP_MISS_RECALL_MIN ), "",
           "| metric | n | misses | AUROC | band reachable | best in-band recall | best in-band false-warn |",
           "| --- | --- | --- | --- | --- | --- | --- |" ]
    for metric in ( "file_hit", "func_hit" ):
        mbp = summary["margin_bp_operating_point"][metric]
        bi = mbp["best_in_band"]
        L.append( "| %s | %d | %d | %s | %s | %s | %s |" %
                  ( metric, mbp["n"], mbp["misses"], md_num( mbp["auroc"] ), mbp["meets_band"],
                    md_num( bi["recall"] ) if bi else "n/a",
                    md_num( bi["false_abstain_rate"] ) if bi else "n/a" ) )
    return L + [ "" ]
...
def print_metric_lines( summary, rows ):
    ...
    for metric in ( "file_hit", "func_hit" ):
        # R-MARGIN: the new signal's own AUROC and whether the pre-registered band (false-warn<=0.20 at
        # miss-recall>=0.50) is reachable by ANY threshold — the negative result the round is designed to
        # be able to publish honestly if it is one.
        mbp = summary["margin_bp_operating_point"][metric]
        print( "LOCBENCH\tcalib\t%s_margin_bp_auroc\t%s" % ( metric, md_num( mbp["auroc"], "%.4f" ) ) )
        print( "LOCBENCH\tcalib\t%s_margin_bp_meets_band\t%s" % ( metric, mbp["meets_band"] ) )
...
def main():
    ...
    summary = dict(
        ...
        # R-MARGIN: the pre-registered operating-point check for the new full-precision signal, over
        # the same two grains as discrimination() above.
        margin_bp_operating_point={ m: margin_bp_operating_point( rows, m ) for m in ( "file_hit", "func_hit" ) },
