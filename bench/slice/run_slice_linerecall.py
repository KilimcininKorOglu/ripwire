#!/usr/bin/env python3
"""run_slice_linerecall.py — the measurement arms of the --slice line-recall round.
...
What it does per carried row:
  ...
  - metrics (a) set recall, (b) Recall@k for R1 / R2 / R2-oracle / random control,
    (c) cost in bytes and wall-clock ms, (d) the §5 fixed-budget granularity comparison.

Usage:
  python3 bench/slice/run_slice_linerecall.py --gold gold.json --bin build/ripwire \
...
def main():
    ...
    summary = {
        ...
        "mrr": { t: m( f"mrr_{t}" ) for t in ( "r0", "r1", "r2", "oracle", "ctl" ) },
    ...
    summary[ "recall_at_k" ] = { t: { k: m( f"{t}@{k}" ) for k in KS } for t in ( "r0", "r1", "r2", "oracle", "ctl" ) }
