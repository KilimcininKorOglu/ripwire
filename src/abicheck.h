...
inline void writeAbiCheck( std::FILE* out, const AbiResult& res, std::size_t maxStructs, bool listAll, std::string_view rootArg = {} )
{
    ...
    rw::emitRaw( out, "<!-- ripwire abi: the cross-branch ABI-BREAK gate — layout(STRUCT) crossed with "
                       ...
                       "counter, nothing is dropped silently. Structs that match are omitted entirely; unrelated= "
                       "counts refs with no merge-base against HEAD at all (unrelated history — never even "
                       "compared, so nothing for them CAN be dropped silently either) and, because such a ref "
                       "never gets the chance to acquire a row, it ALSO lands in quiet= below (the two counters "
                       "are not disjoint: a ref with no rows at all is counted in quiet= whether that is because "
                       "nothing broke or because nothing was ever compared), and a ref whose every row is an excluded "
