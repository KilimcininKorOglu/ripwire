...
inline Graph buildGraph( const IngestResult& ing, const ScipOverlay* scip = nullptr, bool census = false )
{
    ...
    const HashMap<std::string, char> memberFields = Narrower::memberFieldNames( ing );   // Rule 2b's declared-member set, "<Owner>#<field>" (C/C++)
    ...
    for( const Reference& r : ing.references )
    {
        if( !r.isCompose || r.fromSymbol == kNoNode || fieldTypeWrittenInStd( r ) || r.viaArrow )
        {
            continue;   // a type written in namespace std names no in-repo class, whatever its final segment; a smart pointer's pointee is Rule 2b's alone
        }
        const auto it = byName.find( r.calleeName );
        if( it == byName.end() )
        {
