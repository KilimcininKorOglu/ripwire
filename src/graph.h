...
inline Graph buildGraph( const IngestResult& ing, const ScipOverlay* scip = nullptr, bool census = false )
{
    ...
    for( const Reference& r : ing.references )
    {
        ...
        // the receiver's qualified written type decided this site by its last name (Rule 2's narrow, CHA-lite's prune) → every edge is
        // prov="final-segment" (resolve.h finalSegmentTypeAt) — never a class-identity CLAIM, whose one class was verified first
