...
// that decided it wrote the type QUALIFIED — a match on the final segment alone, whose qualifier nothing checked
struct RecvVarType
{
...
struct Narrower
{
    ...
    static constexpr std::size_t kFieldWalkCap = 16;   // total visited names — bounds depth and width together (methodOnTypeOrBases, fieldEntryAt)
    ...
    // "<Owner>#<field>" for every C/C++ member in the field side table (IngestResult::fields). That table, not the typed one, because
    // it holds EVERY declarator shape: the compose capture records no type for `std::optional<Widget> w_;`, and such a member still
    // hides a base's same-named member (fieldEntryAt). Owner is the scope's final segment — the keying of Symbol::scope and chaUp —
    // so same-named classes pool their members: a collision can only add a refusal, never a narrow.
    ...
    //       walk over chaUp does (fieldEntryAt, arm v). Same-NAMED classes
    ...
    // A class that declares no member of that name reads it from its bases (test/fieldnarrowcheck.sh arm v): C++ lookup of a bare
    ...
    RecvVarType recvVarType( const Reference& r ) const
    {
        ...
        if( const auto sit = scopedDecls.byName.find( keyBind ); sit != scopedDecls.byName.end() )
        {
            ...
            return namesStdType( declared.importedName ) ? RecvVarType{} : RecvVarType{ declared.typeName, !declared.importedName.empty() };
        }
        const auto vit = varType.find( keyBind );
        return ( vit == varType.end() ) ? RecvVarType{} : RecvVarType{ vit->second.type, vit->second.writtenQualified };
    }
