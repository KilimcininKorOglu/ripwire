...
// plain constructor call. A plain FUNCTION call is not told apart: `auto x = makeFoo()` records `makeFoo`, and so
// does the assignment `x = makeFoo()` — assignedTypeOf marks that one, which buildGraph keeps only when a class names it.
