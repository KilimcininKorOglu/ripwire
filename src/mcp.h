    ...
    McpDispatchResult out;
    {
        ...
        else if( method == "tools/list" )
        {
            ...
            resp = "{\"jsonrpc\":\"2.0\",\"id\":" + id +
                   ...
                   "{\"name\":\"for\",\"description\":\"Task-lens ranked, signatures-only inventory of the building blocks most relevant to a task (cx=complexity, in=reuse-count). task = the task in plain words; budget_tokens = an optional ceiling (the CLI --for --token-budget). The header carries route= (which ranker answered, and why), confidence=/margin_pct=/margin_bp= (how sharp the ranked head is — low means a starting point, not an answer; margin_bp= is the same gap at full precision, never zeroed) and lens= (the per-row columns this dialect does not measure).\","
