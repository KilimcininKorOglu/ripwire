...
inline constexpr char kHelpHead[] =
        ...
        "                               confidence=\"high|low\" margin_pct=\"N\" margin_bp=\"N\" — derived from the SAME\n"
        "                               relevance-cliff gap statistic --adaptive cuts at (no new scorer, no behavior change;\n"
        "                               the --json dialect carries the same three keys). low means the ranking is FLAT (no\n"
        "                               material score cliff and more positive matches than the head shows) — treat the set\n"
        "                               as a starting point, not an answer; high means a material cliff inside the served\n"
        "                               head (margin_pct= is that drop as a whole percent) or every positive match already\n"
        "                               shown. margin_bp= is the SAME drop at full precision (hundredths of a percent,\n"
        "                               unrounded, scanned across the whole positive-score distribution) and, unlike\n"
        "                               margin_pct=, is never zeroed when the head hits the ceiling — read it when\n"
        "                               margin_pct=\"0\" to see whether the ranking is truly flat or only capped.\n"
