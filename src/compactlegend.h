...
inline constexpr CompactCompletenessTerm kCompactCompletenessTerms[] =
{
    ...
    { "prov",              "prov=scip|binding|import|split|final-segment: how that <c> edge bound (absent: one unique name); split = one arm of an amb= pick; final-segment = a qualified receiver type matched by its last name, namespace unchecked", true },
