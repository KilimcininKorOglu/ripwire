...
for fn in ('keyword_reassign', 'col0_comment', 'survives_after_blind_spot', 'helper_fn'):
...
# --- the degraded parse is DISCLOSED, not merely survived ---
# PRESENCE GUARD. Without it the survival arm above passes vacuously the moment the fixture stops
# hitting an ERROR node (an upstream grammar fix, a fixture edit) — it would be asserting that
# clean files parse cleanly. err= must name all THREE disclosed blind spots: the `%` unique-name
# inside a path, the Godot-3 keyword in statement position, and the column-0 comment in a match.
"$BIN" "$TMP/fix" --skipped --no-cache > "$TMP/skip.xml"
python3 - "$TMP/skip.xml" <<'PYSKIP'
import sys, xml.etree.ElementTree as ET
root = ET.parse(sys.argv[1]).getroot()
rows = {h.get('p'): h for h in root.iter('h')}
b = rows.get('blindspots.gd')
assert b is not None, ('blindspots.gd is not disclosed as degraded at all — the survival arm above '
                       'is vacuous; rows=%s' % sorted(rows))
assert b.get('why') == 'degraded-parse', b.attrib
assert int(b.get('err', '0')) >= 3, (
    'expected >=3 ERROR nodes (one per disclosed blind spot), got err=%s — the fixture stopped '
    'exercising one of them' % b.get('err'))
print('  PASS the blind-spot fixture is DISCLOSED degraded (why=degraded-parse, err>=3)')
PYSKIP

# --- metrics: match arms and word/symbolic boolean joins are counted ---
# DIFFERENTIAL, not a pinned number: `branchy` carries 3 match arms plus `and`/`or`/`&&`/`||`
# joins, `flat` carries none. Without the ingest_metrics.h arms GDScript scores them as ordinary
# nodes and the two functions come out equal, which is the shape this asserts against.
mkdir "$TMP/metrics"
cat > "$TMP/metrics/m.gd" <<'GD'
func flat(a) -> int:
	var t := a
	return t

func branchy(a, b, c) -> int:
	match a:
		0:
			return 1
		"s":
	...
	return 0
GD
"$BIN" "$TMP/metrics" --metrics --no-cache > "$TMP/metrics.xml"
python3 - "$TMP/metrics.xml" <<'PYMET'
import sys, xml.etree.ElementTree as ET
syms = {s.get('n'): s for s in ET.parse(sys.argv[1]).iter('s')}
flat, branchy = syms['flat'], syms['branchy']
fcx, bcx = int(flat.get('cx', '0')), int(branchy.get('cx', '0'))
fcc, bcc = int(flat.get('ccx', '0')), int(branchy.get('ccx', '0'))
assert fcx >= 1, flat.attrib
# 3 match arms + 4 boolean joins (and, or, &&, ||) + 2 ifs, all above `flat`'s baseline.
assert bcx >= fcx + 9, (
    'branchy cx=%d vs flat cx=%d — match arms (pattern_section) and/or boolean joins '
    '(binary_operator with and/or/&&/||) are not being counted for GDScript' % (bcx, fcx))
assert bcc > fcc, ('branchy ccx=%d is not above flat ccx=%d — the match container is not '
                   'raising cognitive complexity' % (bcc, fcc))
assert int(branchy.get('params', '0')) == 3, branchy.attrib
print('  PASS metrics: match arms and word/symbolic boolean joins raise cx/ccx')
PYMET
