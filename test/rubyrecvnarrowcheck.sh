...
for want in 'n="add" sc="Calc"' 'n="add" sc="Tally"' 'n="run" sc="Engine"' 'n="run" sc="Motor"' \
    ...
    grep -q "$want" "$SPLIT" && ok "indexed: $want" || no "the fixture did not index $want — this gate's arms below are meaningless until it does"
done
...
echo "$CL" | grep -q 'n="qualified_call"' && ok "--callers=Calc::add lists qualified_call" || no "--callers=Calc::add does not list qualified_call: $( echo "$CL" | grep -o '<callers[^>]*' )"
...
echo "$CT" | grep -q 'count="0"' && ok "…and reports count=\"0\" (Tally::add has no caller in this tree)" || no "--callers=Tally::add did not report count=0: $( echo "$CT" | grep -o '<callers[^>]*' )"
...
echo "$CE" | grep -q 'n="scoped_call"' && ok "--callers=Engine::run lists scoped_call" || no "--callers=Engine::run does not list scoped_call"
...
echo "$CP" | grep -q 'n="absolute_call"' && ok "--callers=Top::ping lists absolute_call" || no "--callers=Top::ping does not list absolute_call"
...
echo "$MCL" | grep -q 'n="qualified_call"' && ok "mutation: Tally.add → --callers=Tally::add now lists qualified_call" || no "mutation: the edge did not follow the receiver: $MQ"
