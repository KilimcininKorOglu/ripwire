...
while IFS= read -r spec; do
    # here-string, not `printf | grep -q`: grep -q exits at its match while printf is still writing the
    # ~200 KB help, and under this gate's EXIT trap bash 3.2 reports that as a write error (deckcheck §P9)
    if grep -qF -- "    $spec" <<<"$helpText"; then :
