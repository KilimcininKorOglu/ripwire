...
The bind record gains one byte (`kCacheVersion` 22 → 23). `kParserVer` moves 99 → 104 (past the 100–103 the lanes ahead
of this one declare; the landing train assigns it). `test/qschemetrip.hash` is re-pinned, and `test/cachefuzzcheck.sh`'s
