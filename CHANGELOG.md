...
- **A checksum-valid qsnap or qchurn cache blob with a huge record count aborted.** `deserializeSnapshot` and
  ...
  other damage. Gate: `test/cachefuzzcheck.sh`, Part 2's three vector-count rows and the new Part 3. The rows run
...
- **A FIFO, a directory or a device link at `.ripwire_config` or `.ripwire_quality_acks` hung or aborted.** Both
  ...
  through `docparse::detail::readRegularFile`: it opens with `O_NONBLOCK`, asks the descriptor what it opened, and
  reads anything but a regular file as absent, with a stderr line saying so. Gate: `test/crashsweepcheck.sh` B2
  (eight shapes).
