...
`spawn_sh`'s command line, and `openat` on a volume mounted without a drive letter.### Fixed — an ambiguous `--expand` buried its body behind the ranked map, and the escape hatch was stderr-only
