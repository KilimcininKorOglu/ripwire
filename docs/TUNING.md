...
| --- | --- | --- | --- | --- | --- |
| 138 | 137 | 112 | 12 | **37** | 75 |

The first two columns are not the same number, and the gap is not a rounding: `src/` holds **138 cap
declarations** under **137 distinct names** (`kRowCap` declared in more than one file). The sweep
...
every run, which makes them a third population again. Enumerated over the 137 names `src/` declares
today: **111 tunable**, **12 must stay `constexpr`**, and **14 declared since the sweep was
prepared, which no measurement has touched** (`kChurnMergeBombMaxFiles`, `kFieldIdCapacity`,
`kForPageRowsDefault`, `kForPageUnionSymbolCap`, `kMaxAstQueryNesting`, `kMaxBlockBytes`, +8 more).
111 + 12 + 14 = 137, and `emit` refuses to render a partition that does not add up. The `tunable`
column above reads 112 rather than 111 because one name the sweep classified is no longer declared
in `src/` at all: `kSituTestRowsShown`. Quoting "113 of 138" would be wrong in both halves at once,
...
finding, and it says what NOT to do: this is not a 138-cap audit. Most of these constants are
...
`src/mcpverbs.h` — discloses: `blast_radius_capped`, `coboost_commits_capped`, `forgotten_capped`, `hits_capped`, `siblings_capped`, `terms_capped`, `unindexed_candidates_capped` — probe value `128` — **1 verb(s) respond**
