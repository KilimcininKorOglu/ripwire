...
| --- | --- | --- | --- | --- | --- |
| 139 | 138 | 112 | 12 | **37** | 75 |

The first two columns are not the same number, and the gap is not a rounding: `src/` holds **139 cap
declarations** under **138 distinct names** (`kRowCap` declared in more than one file). The sweep
...
every run, which makes them a third population again. Enumerated over the 138 names `src/` declares
today: **111 tunable**, **12 must stay `constexpr`**, and **15 declared since the sweep was
...
`kForPageUnionSymbolCap`, `kMaxAstQueryNesting`, +9 more). 111 + 12 + 15 = 138, and `emit` refuses
...
Quoting "113 of 139" would be wrong in both halves at once, which is the shape of error a generated
...
finding, and it says what NOT to do: this is not a 139-cap audit. Most of these constants are
