...
are not counted as caps, and 219 + 7 is the 226 constants this generator parses out of `src/`.
...
The `class` column below carries that answer where it is known. **120 of 219 caps are classified
(40 INDEXING, 41 OUTPUT, 39 BOUNDARY); the remaining 99 render `—`, which means NOT YET
...
One table for each of the 87 files that declare a cap — the 219 caps counted above, and no parameter.
