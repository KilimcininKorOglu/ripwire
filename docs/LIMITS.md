...
The `class` column below carries that answer where it is known. **122 of 222 caps are classified
(40 INDEXING, 41 OUTPUT, 41 BOUNDARY); the remaining 100 render `—`, which means NOT YET
...
| constant | value | class | note |
| --- | --- | --- | --- |
| `kForPageExtendMaxRows` | `3` | — | extend-3: up to this many rows appended, never a re-sort |
