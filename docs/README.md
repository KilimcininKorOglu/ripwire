...
| **[`research/`](research/)** | Anyone reading a paper we borrowed from, and the authors of one | Research notes: a primitive taken from a paper, measured against that paper's own question on a corpus the product evals do not cover. Each note carries its pre-registration, the commit that fixed the protocol before any number existed, the amendments with dates, and a section naming where our construct is weaker than the one it came from. Not product claims — nothing here is published until an owner pass. |
