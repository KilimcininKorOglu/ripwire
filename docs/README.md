...
| **[`research/`](research/)** | Anyone working on an open question here, or offering to help with one | Investigation branches: a question this tool has NOT answered, the measurement run against it, and the pre-registration that says what would license a change. Not a claim surface — a number here is local evidence for a decision not yet taken, and nothing in it is quoted on `README.md`. Today: `confidence-and-abstention.md`, on whether `--for`'s ranking-confidence disclosure is calibrated well enough to abstain on. |
