...
| **[`research/`](research/)** | Anyone reading — or arguing with — a measurement that is not yet a feature | Investigation notes: a shipped metric specified precisely enough to be re-implemented or refuted, the smells it does not cover, and the corpus numbers behind both, including the ones where the signal turned out to be weak. The scripts that re-run every table live in `bench/aismells/`. Nothing here is a plan. |
