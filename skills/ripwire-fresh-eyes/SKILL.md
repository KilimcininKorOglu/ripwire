...
5. **Ownership / knowledge risk** — `ripwire <dir> --owners[=SYM] --legend=compact`
   ...
   **Per-function readability** — `ripwire <dir> --biggest-first --legend=compact`
   ...
   closed-form model, **largest Halstead volume/token-count/length first** — a size proxy, not a
   readability order (the ordering claim is WITHDRAWN, `docs/EVALS.md` §8): `vol=` Halstead volume,
   `ent=` Shannon token entropy, `lines=` the definition's line span, `posnett=` the fitted sigmoid.
   Complements step 1: `--hotspots` ranks by complexity × *churn* (where pain has been paid), this ranks
   by size, with no git history needed. Two caveats it states in its own legend and you should carry: one
