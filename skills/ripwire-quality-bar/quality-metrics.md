...
| **Nesting depth `nest`** | compounds `ccx`; erosion signal — present in 77% of agent trajectories (SlopCodeBench). **A MAX — misleading solo**: one deep line and a body deep throughout report the same number. | ≤ 3 | `--metrics nest=`, `--lint`, `--quality-delta kind="nesting"` | guard clauses; invert conditions; extract the nested block — read `humps=`/`deep=` first to know which |
| **Nesting profile `humps` / `deep`** | what `nest` throws away: `humps` = maximal regions reaching the nesting bar (exact), `deep` = LINES inside them (floor, `deep_floor="1"`). `deep/loc` separates tangled from blocked-sequential; `deep/humps` separates one giant tangle from many tiny touches. Absent exactly when `nest <` bar — never a hidden `0`; `deep < humps` is legal (two regions can share a line). | judgment — read the two ratios, not either number | `--metrics humps= deep=`, the `structural` `why=` in `--quality-panel`/`--ensemble` | see `ripwire-quality-bar`'s shape → refactor playbook |
| **Local-variable count `locals`** | the working set a reader holds at once — the thing extraction is supposed to shrink. FLOOR (`locals_floor="1"`), **C/C++ only**, absent (never a bare `0`) elsewhere. | compare to the local median | `--metrics locals=`, `--lint --naming-locals` | extract a cohesive block; a "split" that doesn't move `locals` mostly moved braces |
